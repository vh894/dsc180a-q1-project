...
plt.title('Income Distribution and EV Charger Locations Across SDG&E Service Areas');
plt.show()
...
ax.set_facecolor('k')
gdf.plot(ax=ax, color='white', markersize=6)
plt.title('Total Population Distribution and EV Charger Locations Across SDG&E Service Areas');
plt.show()
...
ax.set_facecolor('k')
gdf.plot(ax=ax, color='white', markersize=6)
plt.title('Average Commuting Vehicle Count and EV Charger Locations Across SDG&E Service Areas');
plt.show()

# %%
sd_race_df = products.ACS(2019).from_county('San Diego, CA', level = 'tract', variables = 'B02009_001E')
oc_race_df = products.ACS(2019).from_county('Orange County, CA', level = 'tract', variables = 'B02009_001E')

race_df = pd.concat([sd_race_df, oc_race_df])
race_df.rename(columns={'B02009_001E': 'Count'}, inplace = True)
race_df = race_df.to_crs(epsg=4326)
race_df

# %%
fig, ax = plt.subplots(figsize=(15,15))
ax = race_df.plot('Count', ax=ax, cmap='plasma', legend=True, legend_kwds={'orientation': 'horizontal'})
gdf = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.longitude, df.latitude))

ax.set_facecolor('k')
gdf.plot(ax=ax, color='white', markersize=6)
plt.title('Mapping EV Charger Location and Minority Populations Across SDG&E Service Areas');
plt.show()
